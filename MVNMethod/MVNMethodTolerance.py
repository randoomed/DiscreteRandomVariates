...
class MVNMethodTolerance(object):
    """Implementation of the MVN algorithm by Matias et al. in the paper Dynamic generation of discrete
random variates. This implementation also used to the tolerances specified in the paper."""

    def __init__(self, events, tolerance = 0, c = 0):
        #log*(n) als tolerance = 0.4, c = 12
        ...
        self.minimumNoneRootDegree = 0.5*((2+self.tolerance)/(1-self.tolerance))**2 * 2 ** c
        ...
        while len(ranges) > 0:
            ...
            for r in tmpRanges.values():
                ...
                if len(r) >= self.minimumNoneRootDegree:
                    ranges.append(r)
    ...
    def updateEvent(self, event, newWeight):
        ...
        if event.parent == None:
            ...
            elif len(event) >= self.minimumNoneRootDegree:
            ...
            elif (1-self.tolerance) * 2 ** (event.parent.index) <= newWeight and (2+self.tolerance) * 2 ** (event.parent.index) > newWeight:
                #update the weight of the event
                event.weight = newWeight
    ...
    def _getChildFromRange(self, eventRange):
        ...
        maxWeight = (2 + self.tolerance) * 2 ** (eventRange.index)
        while(True):
            #get random number
    ...
    #print the forest consisting of the trees of all root nodes.
    def showForest(self):
